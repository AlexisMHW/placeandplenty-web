...
export default function ChecklistSignup({ slug, name }: { slug: string; name: string }) {
  ...
  if (result) return (
    <div role="status" className="rounded-card border border-gold/60 bg-cream p-7 shadow-soft sm:p-9">
      ...
      <div className="mt-7 border-t border-sage/40 pt-6">
        ...
        <p className="mt-3 font-body leading-relaxed text-forest/80">Bring your people, menu, and preparation into Place &amp; Plenty. The website is ready when you are. No app download needed.</p>
        <Link href="/signup?next=%2Fhost" onClick={() => track("checklist_account_clicked", { edition: slug })} className="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-full border border-forest px-5 py-3 text-center font-body font-semibold text-forest">Create My Free Website Account</Link>
  ...
  return (
    <form onSubmit={submit} className="rounded-card border border-sage/40 bg-offwhite p-7 shadow-soft sm:p-9">
      ...
      <p className="mt-4 text-center font-body text-sm text-forest/70">Ready to plan now? <Link href="/signup" className="font-semibold underline underline-offset-4">Start free on the website</Link>.</p>
    </form>
  );
}
