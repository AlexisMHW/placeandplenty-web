...
export function generateMetadata({ params }: { params: { edition: string } }): Metadata {
  ...
  return { title, description, alternates: { canonical: `/hosting-checklists/${kit.slug}` }, openGraph: { title: `${title} | Place & Plenty`, description, url: `https://placeandplenty.com/hosting-checklists/${kit.slug}`, images: [kit.image] }, twitter: { card: "summary_large_image", title, description, images: [kit.image] } };
}
export default function ChecklistPage({ params }: { params: { edition: string } }) {
  ...
  return <section className="relative overflow-hidden bg-parchment py-12 md:py-20">
    <BotanicalBough className="pointer-events-none absolute -left-20 bottom-0 text-olive/20" width={220} />
    <div className="relative mx-auto max-w-editorial px-6">
      <Link href="/hosting-checklists" className="font-body text-sm text-forest/70 underline underline-offset-4">All free hosting checklists</Link>
      <div className="mt-7 grid gap-10 lg:grid-cols-2 lg:items-start lg:gap-16">
        <div><p className="font-body text-xs font-bold uppercase tracking-[0.18em] text-olive">The Put-Together Get-Together Starter Kit</p>
          ...
          <Image src={kit.image} alt={`${kit.name} gathering inspiration`} width={800} height={500} priority sizes="(max-width: 1024px) 100vw, 50vw" className="mt-7 aspect-[8/5] w-full rounded-card object-cover" />
