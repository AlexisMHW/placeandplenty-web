...
export default function ChecklistsPage() {
  return <section className="bg-parchment py-14 md:py-20"><div className="mx-auto max-w-editorial px-6">
    ...
    <Link href="/gathering-checklists/before-the-doorbell" className="mt-8 block rounded-card border border-gold bg-cream p-7"><p className="font-body text-xs font-bold uppercase tracking-widest text-forest/75">For every occasion</p><h2 className="mt-3 font-display text-3xl text-forest">The doorbell rings. You’re still in a towel.</h2><p className="mt-3 font-body text-forest/80">Let’s make room for getting yourself ready, too. Get the free Before the Doorbell checklist.</p><span className="mt-4 inline-block font-body font-semibold text-forest underline underline-offset-4">Help Me Beat the Doorbell</span></Link><div className="mt-10 grid gap-7 md:grid-cols-2">{CHECKLISTS.filter(kit => kit.slug !== "before-the-doorbell").map(kit => <Link key={kit.slug} href={`/gathering-checklists/${kit.slug}`} className="overflow-hidden rounded-card border border-sage/40 bg-offwhite shadow-soft transition-shadow hover:shadow-lift">
