...
const JOURNEY: Array<{
  ...
  {
    label: "Start",
    detail: "Create the gathering.",
    icon: "calendar",
  },
  {
    label: "Bring In Your People",
    detail: "Invite, RSVP, organize.",
    icon: "people",
  },
  {
    label: "Plan It",
    detail: "Food, shopping, what you own.",
    icon: "table",
  },
  {
    label: "Get Ready",
    detail: "Know what needs attention next.",
    icon: "gauge",
  },
  {
    label: "Gather",
    detail: "Host, enjoy, remember.",
    icon: "heart",
  },
];
        ...
        <div className="mx-auto max-w-editorial px-5 py-14 md:px-6 md:py-18">
          <div className="text-center">
            ...
            <Display className="mx-auto mt-4 max-w-3xl text-3xl leading-tight text-forest md:text-[2.5rem]">
              One gathering. Five simple moments.
            </Display>
          </div>
          ...
          <div className="relative mt-12 md:mt-14">
            <div
              aria-hidden
              className="absolute left-[10%] right-[10%] top-9 hidden h-[2px] bg-gold/70 md:block"
            />
            <ol className="grid gap-8 md:grid-cols-5 md:gap-4">
              {JOURNEY.map((step, index) => (
                <li key={step.label} className="relative text-center">
                  <div className="mx-auto flex h-[4.5rem] w-[4.5rem] items-center justify-center rounded-full border-2 border-gold bg-parchment text-goldInk shadow-softer">
                    <Icon name={step.icon} size={32} strokeWidth={1.9} />
                  </div>
                  <p className="mt-4 font-body text-[0.62rem] font-bold uppercase tracking-[0.18em] text-goldInk">
                    {String(index + 1).padStart(2, "0")}
                  </p>
                  <h2 className="mt-1 font-display text-xl leading-tight text-forest">
                    {step.label}
                  </h2>
                  <p className="mx-auto mt-2 max-w-[11rem] font-body text-sm leading-snug text-forest/70">
                    {step.detail}
                  </p>
                </li>
              ))}
      ...
      <section aria-label="How Place and Plenty works" className="bg-parchment">
        <div className="mx-auto max-w-editorial space-y-16 px-6 py-16 md:space-y-20 md:py-24">
          {STORY.map((stage, index) => (
            <article
              key={stage.eyebrow}
              className="grid items-center gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-12"
            >
              <div className={index % 2 === 1 ? "lg:order-2" : ""}>
                <div className="flex items-center gap-3">
                  <span className="h-px w-8 bg-gold" aria-hidden />
                  <p className="font-body text-[0.68rem] font-bold uppercase tracking-[0.22em] text-forest/75">
                    {stage.eyebrow}
                  </p>
                </div>
                <h2 className="mt-4 font-display text-3xl leading-tight text-forest md:text-[2.25rem]">
                  {stage.title}
                </h2>
              ...
              <div
                className={`relative aspect-[16/10] overflow-hidden rounded-3xl shadow-soft ${
