...
function GatheringRow({
  ...
  return (
    <li>
      <Link
        href={href}
        className={`group flex items-stretch gap-4 rounded-xl border p-3 transition-all duration-400 hover:-translate-y-px hover:shadow-softer ${
          isDraft
            ? "border-sage/20 bg-parchment/75 hover:border-sage/40"
            : "border-gold/35 bg-parchment hover:border-gold/60"
        }`}
      >
        <GatheringIdentity
          name={gathering.name}
...
export default async function HostHomePage() {
  ...
  const lockedHero = nextLockedIn(listed);
  const draftHero = lockedHero ? null : nextDraft(listed);
    ...
    <div className="mx-auto max-w-[92rem] px-5 py-8 md:px-8 md:py-10">
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
          ...
          {listed.length === 0 ? (
            <div className="mt-6 rounded-xl border border-sage/25 bg-parchment px-5 py-10 text-center">
              ...
              <p className="mt-4 font-display text-lg text-forest">
                Nothing current on the calendar.
              </p>
              <p className="mx-auto mt-2 max-w-sm font-body text-sm leading-relaxed text-forest/70">
                Start a gathering here or in the app. It is the same Place & Plenty account either way.
              </p>
            </div>
          ) : (
        ...
        <div className="space-y-6">
          {lockedHero ? (
            <section className="overflow-hidden rounded-2xl border border-gold/30 bg-cream shadow-softer">
              <div className="grid min-h-[22rem] sm:grid-cols-[minmax(0,1fr)_minmax(16rem,0.82fr)]">
                <div className="flex flex-col justify-center p-7 md:p-8">
                  ...
                  <h2 className="mt-3 max-w-md font-display text-3xl leading-tight text-forest md:text-[2.3rem]">
                    {lockedHero.name}
                  </h2>
                  <span aria-hidden className="mt-4 block h-[2px] w-14 bg-gold" />

                  <p className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 font-body text-sm text-forest/75">
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    <Link
                      ...
                      className="rounded-lg bg-forest px-5 py-2.5 font-body text-sm font-semibold text-offwhite transition-colors duration-400 hover:bg-forest/90"
                    >
                      Open Gathering
                    </Link>
                    <ReadinessBadge
                ...
                <div className="flex items-center justify-center border-t border-gold/20 bg-offwhite p-5 sm:border-l sm:border-t-0">
                  <GatheringIdentity
                    name={lockedHero.name}
                    artworkUrl={artwork.get(lockedHero.id)}
                    className="aspect-square w-full max-w-[22rem] rounded-xl border border-sage/20 shadow-sm"
          ...
          {lockedIn.length === 0 && drafts.length > 1 && (
            <p className="px-1 font-body text-xs leading-relaxed text-forest/55">
              You have {drafts.length} current drafts open. Drafts use an open-gathering slot, but they do not count toward the Plus annual lock-in allowance until you finish the create-gathering flow.
            </p>
          )}
        </div>
      </div>
